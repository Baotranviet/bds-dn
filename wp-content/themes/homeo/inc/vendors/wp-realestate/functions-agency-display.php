...
function homeo_agency_display_nb_properties($post) {
    ...
    <div class="nb-property">
        <?php echo sprintf(_n('%d Bất động sản', '%d Bất động sản', intval($count_properties), 'homeo'), intval($count_properties)); ?>
    </div>
    <?php
}
...
function homeo_agency_display_properties($post, $echo = true) {
    ...
    $output = '<div class="number-properties">'.sprintf(_n('%d Bất động sản', '%d Bất động sản', $count, 'homeo'), $count).'</div>';
    if ( $echo ) {
        echo trim($output);
    } else {
        return $output;
